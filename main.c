...
static void Send( void* context )
{
...
#ifdef CAYENNE_LPP
  uint8_t cchannel=0;
  temperature = ( int16_t )  rand() % 50 ;/*( sensor_data.temperature * 10 );     /* in �C * 10 */
  pressure    = ( uint16_t )  rand() % 1000 ;/*( sensor_data.pressure * 100 / 10 );  /* in hPa / 10 */
  humidity    = ( uint16_t )  rand() % 100 ;/*( sensor_data.humidity * 2 );        /* in %*2     */
  uint32_t i = 0;
  ...
  AppData.Buff[i++] = LPP_DATATYPE_BAROMETER;           //2
  AppData.Buff[i++] = ( pressure >> 8 ) & 0xFF;         //3
  AppData.Buff[i++] = pressure & 0xFF;                  //4
  AppData.Buff[i++] = cchannel++;                       //5  
  AppData.Buff[i++] = LPP_DATATYPE_TEMPERATURE;         //6
  AppData.Buff[i++] = ( temperature >> 8 ) & 0xFF;      //7
  AppData.Buff[i++] = temperature & 0xFF;               //8
  AppData.Buff[i++] = cchannel++;                       //9
  AppData.Buff[i++] = LPP_DATATYPE_HUMIDITY;            //10
  AppData.Buff[i++] = humidity & 0xFF;                  //11
